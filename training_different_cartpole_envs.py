...
pole_mass_mod = args.pole_mass_mod
force_mag_mod = args.force_mag_mod
...
d = date.today()
result_dir = f'Master_Thesis_Code/BP_A2C/difficult_cartpole_envs/{network_type}_a2c_result_' + str(result_id) + "_{}_plm_{}_pmm_{}_fmm_{}_entropycoef_{}_valuepredcoef_{}_\
learningrate_{}_numtrainepisodes_{}_selectionmethod_{}_trainingmethod_{}_numneurons_{}".format(
    str(d.year) + str(d.month) + str(d.day), pole_length_mod, pole_mass_mod, force_mag_mod, entropy_coef, value_pred_coef,
...
for i_run in range(num_models):
    ...
    for section in range(0, int(num_training_episodes/training_eps_per_section)):
        ...
        if section == 0:
            smoothed_scores, scores, best_average, best_average_after, training_total_rewards, training_losses, validation_total_rewards, validation_losses = agent.train_agent_discrete_different_params(training_eps_per_section, section, randomization_params = randomization_params, pole_length_mod=pole_length_mod, pole_mass_mod=pole_mass_mod, force_mag_mod=force_mag_mod)
        else:
            smoothed_scores, scores, best_average, best_average_after, training_total_rewards, training_losses, validation_total_rewards, validation_losses = agent.train_agent_discrete_different_params(training_eps_per_section, section, randomization_params = randomization_params, best_average=best_average_all[i_run], best_average_after=best_average_after_all[i_run], pole_length_mod=pole_length_mod, pole_mass_mod=pole_mass_mod, force_mag_mod=force_mag_mod)
