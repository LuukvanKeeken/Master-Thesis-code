...
def evaluate_BP_policy(policy_net, env, n_evaluations, seeds):
    """ Evaluate a policy on a cartpole environment.
    ...
    for i in range(n_evaluations):
        hebbian_traces = policy_net.initialZeroHebb(1)
        hidden_activations = policy_net.initialZeroState(1)
        ...
        while not done:
            inputs = torch.from_numpy(state).float()
            mem_result, hidden = policy_net(inputs, [hidden_activations, hebbian_traces])
            hidden_activations, hebbian_traces = hidden
            action = torch.argmax(mem_result)
            state, r, done, _ = env.step(action.item())
            reward += r
        eval_rewards.append(reward)
