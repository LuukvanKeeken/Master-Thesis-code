...
parser.add_argument('--num_training_episodes', type=int, default=100000, help='Number of training episodes to run')
...
if result_id == -1:
    ...
    dirs = os.listdir('Master_Thesis_Code/BP_A2C/bipedal_walker/training_results/')
...
result_dir = 'Master_Thesis_Code/BP_A2C/bipedal_walker/Continued_Standard_RNN_a2c_result_68024_202459_entropycoef_0.0001_valuepredcoef_0.0001_learningrate_1e-05_numtrainepisodes_1000000_selectionmethod_exp_BW_validation_trainingmethod_original_numneurons_96'

os.mkdir(result_dir)
print('Created Directory {} to store the results in'.format(result_dir))

weights = torch.load('Master_Thesis_Code/BP_A2C/29_BW_BPandRNN_original_96/Standard_RNN_a2c_result_68024_202459_entropycoef_0.0001_valuepredcoef_0.0001_learningrate_1e-05_numtrainepisodes_1000000_selectionmethod_exp_BW_validation_trainingmethod_original_numneurons_96/checkpoint_BP_A2C_0.pt')
...
for i_run in range(num_models):
    ...
    if network_type == 'BP_RNN':
        agent_net = BP_RNetwork(input_dims, num_neurons, output_dims, seed)
    ...
    for section in range(0, int(num_training_episodes/training_eps_per_section)):
        ...
        if section == 0:
            smoothed_scores, scores, best_average, best_average_after, training_total_rewards, training_losses, validation_total_rewards, validation_losses = agent.train_agent_continuous(training_eps_per_section, section, randomization_params = randomization_params)
        else:
            smoothed_scores, scores, best_average, best_average_after, training_total_rewards, training_losses, validation_total_rewards, validation_losses = agent.train_agent_continuous(training_eps_per_section, section, randomization_params = randomization_params, best_average=best_average_all[i_run], best_average_after=best_average_after_all[i_run])
