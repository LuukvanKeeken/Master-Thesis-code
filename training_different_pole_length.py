...
parser.add_argument('--num_models', type=int, default=20, help='Number of models to train')
parser.add_argument('--selection_method', type=str, default='evaluation_diff_pole_length', help='Method to use for selecting the best model')
...
pole_length_mod = args.pole_length_mod
...
training_seeds = np.concatenate((training_seeds, training_seeds+1))
max_reward = 200
range_min = 0.7
range_max = 1.3
...
if result_id == -1:
    ...
    dirs = os.listdir('Master_Thesis_Code/BP_A2C/difficult_cartpole_envs/')
...
d = date.today()
result_dir = f'Master_Thesis_Code/BP_A2C/difficult_cartpole_envs/{network_type}_a2c_result_' + str(result_id) + "_{}_plm_{}_entropycoef_{}_valuepredcoef_{}_\
learningrate_{}_numtrainepisodes_{}_selectionmethod_{}_trainingmethod_{}_numneurons_{}".format(
    str(d.year) + str(d.month) + str(d.day), pole_length_mod, entropy_coef, value_pred_coef,
...
for i_run in range(num_models):
    ...
    for section in range(0, int(num_training_episodes/training_eps_per_section)):
        ...
        if section == 0:
            smoothed_scores, scores, best_average, best_average_after, training_total_rewards, training_losses, validation_total_rewards, validation_losses = agent.train_agent_discrete_different_pole_length(training_eps_per_section, section, randomization_params = randomization_params, pole_length_mod=pole_length_mod)
        else:
            smoothed_scores, scores, best_average, best_average_after, training_total_rewards, training_losses, validation_total_rewards, validation_losses = agent.train_agent_discrete_different_pole_length(training_eps_per_section, section, randomization_params = randomization_params, best_average=best_average_all[i_run], best_average_after=best_average_after_all[i_run], pole_length_mod=pole_length_mod)
