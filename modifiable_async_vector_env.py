...
class ModifiableAsyncVectorEnv(VectorEnv):
    ...
    def __init__(self, env_fns, observation_space=None, action_space=None,
        ...
        if (observation_space is None) or (action_space is None):
            ...
            dummy_env.close()
            del dummy_env
        super(ModifiableAsyncVectorEnv, self).__init__(num_envs=len(env_fns),
...
def _worker(index, env_fn, pipe, parent_pipe, shared_memory, error_queue):
    ...
    try:
        while True:
            ...
            elif command == 'set_env_params':
                for param_name, value in data.items():
                    env.unwrapped.__setattr__(param_name, value)
                pipe.send((None, True))
            else:
