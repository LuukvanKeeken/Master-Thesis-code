...
class A2C_Agent:
    ...
    def train_agent_discrete_testing_ranges(self, training_eps_per_section, section, randomization_params = None, randomize_every = 5, best_average = -np.inf, best_average_after = np.inf):
        # best_average = -np.inf
        # best_average_after = np.inf
        scores = []
        smoothed_scores = []
    ...
    def train_agent_discrete_different_pole_length(self, training_eps_per_section, section, randomization_params = None, randomize_every = 5, best_average = -np.inf, best_average_after = np.inf, pole_length_mod = 1.0):
        # best_average = -np.inf
        # best_average_after = np.inf
        scores = []
        smoothed_scores = []
        ...
        for episode in range(1 + section*training_eps_per_section, (section+1)*training_eps_per_section + 1):
            ...
            self.env = gym.make(self.env_name)
            self.env.unwrapped.length *= pole_length_mod
            ...
            for steps in range(self.max_steps):
                ...
                if done or steps == self.max_steps-1:
                    ...
                    if ((self.selection_method == "evaluation_diff_pole_length") and (episode % self.evaluate_every == 0)):
                        evaluation_performance = np.mean(evaluate_BP_agent_pole_length(self.agent_net, self.env_name, self.num_evaluation_episodes, self.evaluation_seeds, pole_length_mod))
                        print(f"Episode {episode}\tAverage evaluation: {evaluation_performance}")
                        validation_total_rewards.append(evaluation_performance)
                    ...
                    elif ((self.selection_method == "evaluation") and (episode % self.evaluate_every == 0)):
                        evaluation_performance = np.mean(evaluate_BP_agent_pole_length(self.agent_net, self.env_name, self.num_evaluation_episodes, self.evaluation_seeds, 1.0))
                        print(f"Episode {episode}\tAverage evaluation: {evaluation_performance}")
                        validation_total_rewards.append(evaluation_performance)
    ...
    def train_agent_discrete_different_params(self, training_eps_per_section, section, randomization_params = None, randomize_every = 5, best_average = -np.inf, best_average_after = np.inf, pole_length_mod = 1.0, pole_mass_mod = 1.0, force_mag_mod = 1.0):
        # best_average = -np.inf
        # best_average_after = np.inf
        scores = []
        smoothed_scores = []
        ...
        for episode in range(1 + section*training_eps_per_section, (section+1)*training_eps_per_section + 1):
            ...
            self.env = gym.make(self.env_name)
            self.env.unwrapped.length *= pole_length_mod
            self.env.unwrapped.masspole = pole_mass_mod
            self.env.unwrapped.force_mag = force_mag_mod
            ...
            for steps in range(self.max_steps):
                ...
                if done or steps == self.max_steps-1:
                    ...
                    if ((self.selection_method == "evaluation_diff_pole_length") and (episode % self.evaluate_every == 0)):
                        evaluation_performance = np.mean(evaluate_BP_agent_all_params(self.agent_net, self.env_name, self.num_evaluation_episodes, self.evaluation_seeds, pole_length_mod, pole_mass_mod, force_mag_mod))
                        print(f"Episode {episode}\tAverage evaluation: {evaluation_performance}")
                        validation_total_rewards.append(evaluation_performance)
