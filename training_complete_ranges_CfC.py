...
parser.add_argument('--entropy_coef', type=float, default=100.0, help='Entropy coefficient for the agent')
parser.add_argument('--value_pred_coef', type=float, default=0.01, help='Value prediction coefficient for the agent')
parser.add_argument('--num_training_episodes', type=int, default=30000, help='Number of training episodes to run')
...
if result_id == -1:
    dirs = os.listdir('Master_Thesis_Code/LTC_A2C/difficult_cartpole_envs/')
...
d = date.today()
result_dir = f'Master_Thesis_Code/LTC_A2C/difficult_cartpole_envs/{network_type}_a2c_result_' + str(result_id) + f'_{str(d.year)+str(d.month)+str(d.day)}_learningrate_{learning_rate}_selectiomethod_{selection_method}_trainingmethod_{training_method}_numneurons_{num_neurons}'
if network_type == "CfC":
