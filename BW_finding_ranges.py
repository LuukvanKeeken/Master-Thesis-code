...
def main():
    with torch.no_grad():
        ...
        policy_weights = torch.load("Master_Thesis_Code/BP_A2C/bipedal_walker/BP_and_StandardRNN/Standard_RNN_a2c_result_9000002_2024613_entropycoef_0.01_valuepredcoef_0.01_learningrate_0.0001_numtrainepisodes_1000000_selectionmethod_original_trainingmethod_original_numneurons_96/checkpoint_BP_A2C_0.pt")
        ...
        for variable, default_value in zip(variables, default_values):
            with open(f"Master_Thesis_Code/current_variable_2.txt", 'w') as f:
                f.write(f"{variable}")
            ...
            np.save("Master_Thesis_Code/BW_finding_ranges_means_STDRNN_2.npy", all_results_means)
            np.save("Master_Thesis_Code/BW_finding_ranges_stds_STDRNN_2.npy", all_results_stds)
